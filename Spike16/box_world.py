...
from vector2d import PointToVector2D
...
#                print i, j, j%nx
    ...
    def set_agent_start(self, agent_idx, idx):
        ...
        self.start[agent_idx] = self.boxes[idx]
        ...
        if self.path is not None:
            for agent_idx, agent in enumerate(self.agent):
                if agent is not None:
                    ...
                    new_path_pts = [PointToVector2D(self.boxes[p]._vc) for p in self.path.path]
    ...
    def plan_path(self, search, limit, agent_idx, speed=None):
        ...
        path_pts = [PointToVector2D(self.boxes[p]._vc) for p in self.path.path]
        if self.agent[agent_idx] is None:
            start_pos = PointToVector2D(self.boxes[start.idx]._vc)
            self.agent[agent_idx] = Agent(start_pos, speed=speed if speed is not None else 5) # default speed is 5
    ...
    def FromFile(cls, filename, pixels=(500,500) ):
        ...
        start_targets = [int(bit) for bit in lines.pop(0).split()]
        t_idx = start_targets.pop()
        for s_idx in start_targets:
            world.add_agent(s_idx)
